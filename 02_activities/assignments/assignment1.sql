...
SELECT product_id, vendor_id, market_date, customer_id, transaction_time,
...
SELECT vendor_id, 
COUNT(booth_number) AS rent_count
...
SELECT c.customer_id, customer_first_name, customer_last_name, customer_postal_code
,SUM(quantity * cost_to_customer_per_qty) AS all_purchases
FROM customer AS c
INNER JOIN customer_purchases AS cp
	ON c.customer_id = cp.customer_id
GROUP BY c.customer_id
HAVING all_purchases >= 2000
ORDER BY c.customer_last_name ASC, c.customer_last_name ASC;
...
*/
-- if a table named new_vendor exists, delete it, otherwise, do nothing
...
SELECT *
FROM vendor;
-- add the new vendor into the temp table
